...
template<class T>
inline Triangle<T>::Triangle(T p1, T p2, T p3)
{
	points[0] = p1;
	points[1] = p2;
	points[2] = p3;
...
template<class T>
inline void Triangle<T>::DrawCall(Graphics& ghs)
{
...
template<class T>
DirectX::XMFLOAT3 Triangle<T>::InTriangle(float x, float y)
{
...
template<class T>
bool Triangle<T>::BackFaceCulling()
{
